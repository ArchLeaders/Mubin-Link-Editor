...
class RegisterHelper:
            ...
            for item in categories[category]:
                ...
                collect.append(
                    NodeItem(
                        ...
                        label=item,
                        settings={
                            "definition": repr(item.replace("New", "LinkTagAnd")),
                            "scale": repr(scale),
                        },
                    )
                )
    ...
    def data(
        set: str = "config", type: str = "json", write: bool = False, write_data={}
    ):
        if write:
            if type.lower() == "json":
                Path(
                    f'{os.environ["LOCALAPPDATA"]}\\mubin_link_editor\\{set}.json'
                ).write_text(json.dumps(write_data, indent=4))
            elif set.lower() == "yaml":
                yaml.dump(
                    write_data,
                    Path(
            ...
            if type.lower() == "json":
                ...
                return yaml.load(
                    Path(
                        f'{os.environ["LOCALAPPDATA"]}\\mubin_link_editor\\{set}.yml'
                    ).open()
                )
...
class NodeHelper:
    ...
    def import_actor_merged(actor, scale=(1, 1, 1)):
        ...
        bpy.ops.wm.collada_import(
            filepath=f'{exp}\\{actor["BfresName"]}\\{actor["ModelName"]}.dae'
        )
        ...
        for field in fields:
            for let in letters:
                for num in numbers:
                    for type in types:
                        file = Path(
                            f"{aoc_dir}\\Map\\{field}\\{let}-{num}\\{let}-{num}_{type}.smubin"
                        )
...
class Constructor:
    ...
    def build_template(base: bpy.types.NodeLink, tree: bpy.types.NodeTree):
        ...
        Constructor.json_data = {"name": tree.name, "actors": []}
    ...
    def parse(value):
        ...
        return {"type": dtype, "value": dvalue}

    def format_vector(
        vector: typing.List[float], compare: bool = True, xzy: bool = False
    ) -> dict:
        ...
        if first == static_first and compare:
            ...
            json_data = [
                {"type": 210, "value": vector[0]},
                {"type": 210, "value": vector[2]},
                {"type": 210, "value": vector[1]},
            ]

        return json_data

    def build_params(exporter: bpy.types.Node):
        """Returns a IS formatted parameter list"""

        json_data = {}

        for node in exporter:
            json_data[node.name] = Constructor.parse(node.value)
    ...
    def constructor(base: bpy.types.NodeLink):
        ...
        for node in base.outputs[0].links:
            ...
            for link in node.outputs[0].links:
                ...
                link = link.to_node
                for actor in link.outputs[0].links:

                    actor = actor.to_node
                    if actor.name not in Constructor.handled:
                        Constructor.hash_id = Constructor.hash_id + 1
                        id = Constructor.hash_id
                    else:
                        id = Constructor.handled[actor.name]

                    links.append(
                        Constructor.omit_null(
                            {
                                ...
                                "DestUnitHashId": {"type": 211, "value": f"{{ID{id}}}"},
                            }
                        )
                    )
            ...
            if node.name not in Constructor.handled:
                Constructor.handled[node.name] = Constructor.hash_id
                ...
                if node.ref_object:
                    Constructor.reset_origin(node.ref_object)
                ...
                Constructor.json_data["actors"].append(
                    Constructor.omit_null(
                        {
                            "!Parameters": Constructor.build_params(node.params),
                            "HashId": {"type": 211, "value": f"{{ID{self}}}"},
                            "LinksToObj": links,
                            "Rotate": Constructor.format_vector(rotate),
                            "Scale": Constructor.format_vector(scale),
                            "SRTHash": {"type": 209, "value": 0},
                            "Translate": Constructor.format_vector(
                                translate, False, True
                            ),
                            "UnitConfigName": {"type": 160, "value": node.definition},
                        }
                    )
                )
