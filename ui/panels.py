...
class MLE_SETTINGS_PT_Nodes(Panel, MLEBase):
    ...
    def draw(self, context):
        row = self.layout.row()
        row.template_list("MLE_UL_Params", "Nodes", context.scene, "nodes", context.scene, "nodes_index")
        col = row.column()
        col.ui_units_x = 1.0
...
class MLE_SETTINGS_PT_Ignored(Panel, MLEBase):
    ...
    def draw(self, context):
        row = self.layout.row()
        row.template_list("UI_UL_list", "Ignore", context.scene, "ignore", context.scene, "ignore_index")
        col = row.column()
        col.ui_units_x = 1.0
